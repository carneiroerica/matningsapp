...
# Load CSV file (optional: for reference)
...
df = load_data("Matningsdata.xlsx")
...
if st.button("Beräkna kostnadsfördelning"):
    if total_kostnad > 0:
        if "Andel" not in df.columns:
            ...
            st.dataframe(df[["DP (TPAB)", "Månadsvisa rör", "Kvartalsvisa rör", "Årsmätningar", "Andel", "Beräknad kostnad"]])
